...
func (u UnixIO) ReadMemo(file *File, blockdata []byte, column *Column) ([]byte, bool, error) {
	...
	if isEmptyBytes(blockdata) {
		return []byte{}, false, nil
	}
	// Determine the block number and classify the pointer before touching the
	// FPT offset: free/header blocks and pointers past the file end are
	// distinct failures.
	block := binary.LittleEndian.Uint32(blockdata)
	fptSize, sizeErr := seekSize(relatedHandle)
	if sizeErr != nil {
		return nil, false, NewError("failed to determine FPT size").Details(sizeErr)
	}
	if err := validateMemoAddress(file.memoHeader, fptSize, blockdata); err != nil {
		return nil, false, WrapError(err)
	}
	...
	if sign == 1 || !column.Flag.Has(byte(BinaryFlag)) {
		...
		if err != nil {
			return buf, sign == 1, NewError("failed to decode memo data").Details(fmt.Errorf("%w: %v", ErrInvalidEncoding, err))
		}
	}
...
func (u UnixIO) WriteMemo(address []byte, file *File, raw []byte, text bool, length int) ([]byte, error) {
	...
	// A new memo is appended at NextFree; rewriting an existing address keeps
	// NextFree stable and only rewrites that block in place.
	blocks := 1
...
func (u UnixIO) ReadRow(file *File, position uint32) ([]byte, error) {
	...
	if err != nil {
		return nil, NewError("failed to determine DBF size").Details(err)
	}
...
func (u UnixIO) WriteRow(file *File, row *Row) error {
	...
	// Record bytes are written first; the header count is committed only
	// afterwards so a failed write never advertises records that do not exist.
